...
let getComment=async(item,id, cont)=>{
		...
		for (let i = cont.children.length - 1; i >= 0; i--) {
			console.log(item)
			item.length!==0 && item[i].clientHeight < item[i].scrollHeight
			...
			knifeItem.innerHTML+=`
							<div class="item_knife">
									...
									<div class='desc_knife ${i.stock===0?" desc_kni_gray":''}' data-id='${i.id}' data-type='${i.type}'>
										...
										<div class="price_knife">${i.price}₽</div>
			...
			plase.innerHTML=`
				<div class="only_desc">		
							<div class="header_desc">
								<div class="img">
									...
									<div class="imges imgig">
										<div class="only_img0"><img width='100%' heigth='100%' src="${res.img[0].item}" alt="pic"></div>
										<div class="only_img0 disen" ><img width='100%' heigth='100%' src="${res.img[1].item}" alt="pic"></div>
										<div class="only_img0 disen"><img width='100%' heigth='100%' src="${res.img[2].item}" alt="pic"></div>
										<div class="only_img0 disen"><img width='100%' heigth='100%' src="${res.img[3].item}" alt="pic"></div>
									</div>

									<div class="imges imsmall">
										<div class="only_img"><img width='100%' heigth='100%' src="${res.img[0].item}" alt="pic"></div>
										<div class="only_img"><img width='100%' heigth='100%' src="${res.img[1].item}" alt="pic"></div>
										<div class="only_img"><img width='100%' heigth='100%' src="${res.img[2].item}" alt="pic"></div>
										<div class="only_img"><img width='100%' heigth='100%' src="${res.img[3].item}" alt="pic"></div>
									</div>
								</div>
								...
								<div class="desc_list_config">
									...
									<ul>
										<li><div>Тип:</div><div>${res.type}</div></li>
										<li><div>Заточка:</div><div>${res.sharpening}</div></li>
										<li><div>Длина(БР):</div><div>${res.lengthb}</div></li>
										<li><div>Длина(СР):</div><div>${res.length}</div></li>
										<li><div>Ручка:</div><div>${res.handle}</div></li>
										<li><div>Вес:</div><div>${res.heft}</div></li>
									</ul>
							...
							<div class="text_desc">
								...
								<div class="but_add acti_but ${res.stock>0?'acti':'disen'} " data-stock='${res.stock}' data-type="${res.type}" data-id="${res.id}">Добавить в корзину</div>
							  <div class="but_add ${res.stock<=0?'acti':'disen'} ">Продукт на складе отсутствует</div>
	 	...
	 	cardItem.innerHTML+=`
			<div class="item_chose_product">
					...
					<div class="desc_chose">
						<h3>${x.id}-${x.type}</h3>
						<div>Длина:  ${x.length}</div>
						<div>Ручка:  ${x.handle}</div>
					</div>
					...
					<div class="price_chose">
						<div>
							<div data-price='${x.price}'>${x.price}₽</div>
						...
						<div class='balans-item' >
								<div>Итого:</div><div class="item-summ" data-type="${x.type}" data-id="${x.id}" data-price='${x.price}'>${x.num*x.price}</div><div>₽</div>
						</div>
					</div>
			<div>
	 	`
